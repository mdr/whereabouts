/*
 * Vendored from rivalis (https://github.com/kalevski/rivalis), node/src/WSTransport.ts
 * at commit of v8.0.0. Copyright (c) 2023 Daniel Kalevski. MIT License.
 *
...
import { CloseCode } from "@rivalis/core";
...
class WSTransport extends Transport {
  ...
  constructor(
    ...
    queryTicketParam: string | null = null,
    ...
    const resolvedMaxPayload = transportOptions.maxPayload ?? options.maxPayload ?? DEFAULT_MAX_PAYLOAD;
  ...
  private handleConnect = async (socket: WebSocket, request: IncomingMessage): Promise<void> => {
    ...
    this.transportLayer.on("message", aid, (_, message) => {
      ...
      if (
        checkBackpressure(aid, socket.bufferedAmount, this.maxBufferedBytes, this.onBackpressureDrop, (msg) =>
